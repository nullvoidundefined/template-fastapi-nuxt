...
endpoints. It is drawn from the code on the branch that carries slices 01 through 05 and slice 07
(observability and integrations); slice 06 (billing) has not merged here, so it adds no events,
codes, or log lines to this catalog yet.
...
client is a no-op and logs `analytics_disabled` once at startup.
...
exceeded reaching R2 to validate credentials.
...
| Worker  | `GET /health` (on `WORKER_PORT`, default 3002) | `GET /health/ready` (on `WORKER_PORT`) | Readiness runs `SELECT 1` against Postgres and a Redis `PING`, concurrently, each with a 2-second timeout; liveness touches nothing |
