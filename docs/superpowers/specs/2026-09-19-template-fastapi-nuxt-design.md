...
**Request path.** The browser talks only to the Nuxt origin. A page request is rendered by Nuxt. `api/apiClient.ts` exports `createApiClient()`, which builds an openapi-fetch client typed by `packages/api-types`, and a `useApiClient()` composable memoizes one client per Nuxt app instance on `useNuxtApp()`, which on the server means one per request. There is never a module-level client, because on the server a module outlives the request, and a client holding one request's cookie would send it on every later user's server-side render. In the browser the client's base URL is `/api`. On the server it is `runtimeConfig.apiBaseUrl` plus `/v1`, FastAPI reached directly over the private network, and the client's headers carry the incoming cookie from `useRequestHeaders(['cookie'])` and the client's address as a single-value `X-Forwarded-For`, built by the same function the Nitro proxy uses, so a server-side call is authenticated as the user and rate-limited in the user's own bucket rather than Nitro's. `useRequestFetch()` is not passed as openapi-fetch's `fetch`: openapi-fetch calls a standard `fetch` and reads a `Response`, while `useRequestFetch()` returns Nuxt's `$fetch`, which resolves to the parsed body (corrected 2026-09-19, after the agent-governance review of the convention tracks). Browser calls to `/api/**` go through the Nitro catch-all `server/api/[...path].ts`, which proxies to FastAPI's `/v1/**`. PostHog traffic goes through `server/api/ingest/[...path].ts`.

**Client IP trust chain.** Every browser request reaches FastAPI through Railway's edge and then Nitro, so FastAPI's direct peer is always Nitro. Railway's edge appends the connecting client's address as the last `X-Forwarded-For` entry; every earlier entry is client-supplied and untrusted. The Nitro proxy therefore forwards only that last entry, as a single-value `X-Forwarded-For`, and drops the rest. FastAPI honors forwarded headers only from Nitro: uvicorn runs with `--proxy-headers --forwarded-allow-ips` set to the web service's private-network address (`TRUSTED_PROXY_IPS`), so a request from anywhere else is keyed on its peer address. The rate limiter reads the resolved `request.client.host` and never parses the header itself.
...
- **Data.** One function per backend route in `app/api/`, each a one-line call on the openapi-fetch client from `useApiClient()`, so its request and response types come from the OpenAPI document; every query behind a TanStack Query composable (`useSessionQuery`, `useCheckoutMutation`, and so on); `useState` composables hold only the theme and the modal stack, never server data.
