...
**Approach:** Add `app/export_openapi.py`, which builds the app and writes `apps/server/docs/openapi.yaml`. Add `packages/api-types` with a `generate` script that runs openapi-typescript (a generator that turns an OpenAPI document into TypeScript types) over that file into `src/schema.ts`. Commit both generated files. In the Nuxt app, `app/api/apiClient.ts` creates one openapi-fetch client (a small typed fetch wrapper that reads the same generated types, chosen in the spec's stack audit) exposed as `createApiClient({ baseUrl, headers })`, so every later route call is typed with no per-route typing by hand. Scope change during the build: the client takes forwarded headers instead of `useRequestFetch()`, which openapi-fetch cannot use as its fetch function; the spec's Request path section is amended to match, and the `useApiClient()` composable that passes `useRequestHeaders(['cookie'])` lands with slice 03's first server-rendered API call.
...
**Contents:** `apps/server/app/export_openapi.py`, `apps/server/docs/openapi.yaml`, `packages/api-types/` (package manifest, generate script, generated `src/schema.ts`), `apps/client/web/app/api/apiClient.ts`, `scripts/check-contract-drift.sh`, and tests.
