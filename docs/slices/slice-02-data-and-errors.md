...
Every other Redis failure in production is one branch, not two, and production never takes the in-memory path on any of them, because a per-replica count multiplies the effective limit by the replica count and a client can reset it by reconnecting. That covers both a connection lost after a successful start and a first connection that never succeeds although `REDIS_URL` is set, which settings validation cannot catch because it checks that the value is present, not that the server answers. In either case the four auth paths fail closed and answer 503 `SERVER_RATE_LIMIT_UNAVAILABLE`, while every other route is served with one error log carrying `rate_limiter_unavailable`, so a Redis outage costs four endpoints rather than the API. The in-memory counter exists only for `environment="test"` and development, and no production code path reaches it. The log line is the whole signal in this slice: Sentry arrives in slice 07 under B-30, and inventing a reporting abstraction here to be filled in later would be unused code (R-309). Slice 07 adds the report when the client exists, and the Later list carries it. The owner settled this on 2026-09-20 after the review found the behavior asserted in the review focus but defined nowhere; the spec's Failure modes section carries the same wording.
