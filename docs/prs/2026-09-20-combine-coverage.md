...
`[tool.coverage.run]` gains `parallel` and `relative_files`. The first keeps the two suites' data files from overwriting each other; the second stops each runner's absolute working directory being baked into the recorded paths, which would make the two sets of data refer to files that never match.
...
**`COVERAGE_FILE` per suite rather than renaming after the run.** Discovered by testing: pytest-cov erases `.coverage*` when it starts, so a file renamed out of the way by one run is deleted by the next. In CI the two suites run on separate runners and would not collide, but the same commands have to work locally, and a developer running both in one checkout would otherwise combine one file and never know.
...
Verified by running the sequence locally. The unit suite wrote `.coverage.unit`, the integration suite wrote `.coverage.integration` against the compose Postgres 17 and Redis 7, `coverage combine` reported `Combined 2 files`, and the combined report gave 97 percent over 316 statements against the 80 percent floor. The first attempt reported `Combined 1 file` and a false 59 percent, which is what exposed the erase behavior described above.
...
What I got wrong first: I assumed renaming `.coverage` between runs was enough and wrote the local check that way. It reported `Combined 1 file` and a total of 59 percent, which I could have read as a genuine coverage problem and gone looking for uncovered modules. The thing that caught it was asserting on the combine count rather than on the percentage, and that is why the CI step now does the same.
