...
The behavior was verified directly instead. The root configuration lints exactly the eleven intended files, confirmed by reading `eslint . --format json` rather than trusting a zero exit code, which a configuration matching nothing also returns. The same reading, taken with a probe file planted under `.claude/`, is what caught the ignore bug the pre-merge review found: flat config does not skip dot-directories, so before `.claude/` was ignored a checkout holding a parallel-session worktree linted that worktree's web sources under the root rules and its generated schema with them. A deliberate unused variable added to `e2e/health.spec.ts` was reported as an error and the file was reverted, so the configuration is known to fail on a real violation rather than only to load. `pnpm lint` passes across the root and the web package. The three images were built and the stack started with `docker compose up --no-build`, proving that the explicit image names in compose match the tags the composite action loads, which is the one way this change could break the two jobs it touches.
