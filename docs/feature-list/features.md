...
| Nitro API proxy and per-request IDs on page requests                                                    | **Complete** | US-AUTH-003; the query string survives, `Set-Cookie` propagates, the forwarded chain is replaced by the edge-appended address, and client forwarding headers are stripped     |
